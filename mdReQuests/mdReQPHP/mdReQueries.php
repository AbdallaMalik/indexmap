<?php 	
	function mdTVLOads($mdQuery,$dfKNd,$md)
	{
		...
		while($tvROWs = mysqli_fetch_assoc($mdQuery))
		{
			...
			$imgUrl   = dhPROtYMAInPhoto($PromediaId,"property","tv");
			//$PimQR    = $smURLPARENt."homes/tv-assets/qr.png";

			$PimQR = mdUSQRcode($tUsrID,$tUsrLoGIn);
	...
	function dhPROtYMAInPhoto($PId,$key,$tYPe)
	{
		global $smURLPARENt, $stDBss;
		$imgUrl = "";
		if($tYPe == "property")
	...
	function dhPROtYMAInTvSLiders($PId,$Prms,$Pbth,$Pkit,$PGarage,$PNme,$PsTYPE,$Psize,$mdPRCe,$mdPRCePMT,$mdPRCePFT,$Ptels,$Pmls,$PimQR,$PimUsr,$Pstus,$PCategory,$PTYPe,$PctrY,$PctY,$Padrs,$mdCURNCYname,$themeMD,$mdGPcounter)
	{
		...
		?>
		<div class="mdTVTheme2 mdTVThemes overlay_layerTVmBODY overlay_layerTVmBODY<?php echo $mdGPcounter;?>">
		<?php 
        if($themeMD == "theme1")
		{
        ?>		
	...
	function sPchTVReadPARAmeters($PId,$sPechPROPERtY,$Pcategory,$PsTYPE,$Pstus,$PPrce,$Psize,$Prms,$Ptels,$Pmls,$PimUsr,$themeMD,$mdGPcounter,$mdthstart,$mdthend)
	{
		...
		else
		{
			...
			?>
			<article class="mdsPeechARTIcLeTv speechARTIcLeTv speechARTIcLeTv<?php echo $mdGPcounter;?> <?php echo $themeMD;?>" id="<?php echo $mdGPcounter;?>" md="<?php echo $themeMD;?>" mdthstart="<?php echo $mdthstart;?>" mdthend="<?php echo $mdthend;?>" class="form-control bg-dark text-light mt-5">
	...
	function mdUSQRcode($uSerID,$aUserLoG)
	{
	    ...
	    else
	    {
		    ...
		    $codeLOGN = $smURLclub."members/profile.php?id=".$aUserLoG;
            $codeName = $codeLOGN;	
